...
# Add numba import with fallback
try:
    import numba
    ...
    HAS_NUMBA = False
    print("Warning: numba not available, running standard benchmarks only")


class Timer:
    """Simple timer to replace pyperf functionality"""
...
def run_benchmark(benchmark_name, repetitions=3):
    ...
    func_args = BENCHMARKS[benchmark_name]
    func = func_args[0]
    args = func_args[1:]
    ...
    for i in range(repetitions):
        ...
        elapsed = func(1, *args)  # Set loops=1 for a single run
    ...
    if len(times) > 1:
        ...
        print(
            f"Results: avg={avg_time:.6f}s, stdev={std_dev:.6f}s, min={min(times):.6f}s, max={max(times):.6f}s"
        )
    else:
        print(f"Results: time={avg_time:.6f}s")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run SciMark benchmarks standalone")
    parser.add_argument(
        "benchmark",
        nargs="?",
    ...
    if args.benchmark and args.benchmark != "all":
        run_benchmark(args.benchmark, args.repetitions)
    else:
        for bench in sorted(BENCHMARKS):
            run_benchmark(bench, args.repetitions)
            print()  # Add an empty line between benchmarks
